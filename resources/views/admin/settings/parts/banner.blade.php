    ...
    <form id="Form_logo" method="post" action="{{route('settings.update',$settings->id)}}">
        ...
        <div class="form-group row mb-4">
            <label for="header_logo" > الصورة
                ...
                <input type="file"
                       data-default-file="{{get_file($settings->banner_logo)}}"
                       class="form-control dropify" id="banner_logo" name="banner_logo"
        ...
        <div class="form-group row mb-4">
                    ...
                    <select multiple  name="countries_banner[]" class="form-control select2Users">
                        @foreach($countries as $country)
                            <option value="{{$country->id}}" @if(in_array($country->id,json_decode($settings->countries_banner))) selected @endif >{{$country->title}}</option>
                        @endforeach
                    </select>
