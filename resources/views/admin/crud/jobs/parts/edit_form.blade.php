<form action="{{route('jobs.update',$obj->id)}}" method="post" id="Form">
    @csrf
    @method('PUT')
        ...
        @foreach($languages as $index=>$language)
            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
                <div class="form-group">
                    <label for="{{$language->title}}_title">{{__('admin.Job Title')}} {{--({{__('admin.'.$language->title)}}
                        )--}} </label>
                    <input data-validation="required" type="text" class="form-control"
                           ...
                           placeholder="{{__('admin.Job Title')}} {{--({{__('admin.'.$language->title)}}) --}}">
                </div>
            </div>
        @endforeach
