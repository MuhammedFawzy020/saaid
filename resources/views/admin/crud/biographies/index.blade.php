...
@section('page-title')
    {{ $value == 'rental' ? 'السير الذاتية للايجار' : ($value === 'serviceMove' ? 'السير الذاتية لنقل الخدمات' : 'السير الذاتية') }}
@endsection
                ...
                <div class="card-body collapse show">
                    <div class="row">
                                ...
                                <select class="form-control " name="nationality_id" id="nationality_id">
                                    ...
                                    @foreach ($natinalities as $key => $country)
                                        <option value="{{ $country->id }}"
                                            @if ($nationality_id == $country->id) selected @endif>
                                            {{ $country->title }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control " name="recruitment_office_id" id="recruitment_office_id">
                                    ...
                                    @foreach ($recruitment_office as $key => $office)
                                        <option value="{{ $office->id }}"
                                            @if ($recruitment_office_id == $office->id) selected @endif>
                                            {{ $office->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-2 text-end">
                            @if (count($_GET) > 0)
                                <a id="cancel_request" href="{{ route('biographies.index', $value) }}"
                    ...
                    <thead>
                        <tr>
                            <th>
                                ...
                                <a id="bulk_active" href="#" style="display:none;" class="text-success p-2"
                                    title="تفعيل العرض">
                                    <i class="mdi mdi-eye-outline me-1"></i>
                                </a>
                                <a id="bulk_inactive" href="#" style="display:none;" class="text-warning p-2"
                                    title="إخفاء">
    ...
    <script>
        ...
        $("#btnSubmit").click(function() {
            if ($("#cancel_request").html() == undefined && $('.cancel_request_add').hide()) {
                $('<a href="{{ route('biographies.index', $value) }}" class="btn btn-danger cancel_request_add" style="margin:5px;">إلغاء البحث</a>')
        ...
        $(document).on('click', '#bulk_inactive', function(e) {
            ...
            $.ajax({
                ...
                success: function() {
                    ...
                    cuteToast({
                        type: "success",
                        message: "تم إخفاء السير الذاتية بنجاح",
                        timer: 2500
                    });
                    datatable_selector.draw();
                },
