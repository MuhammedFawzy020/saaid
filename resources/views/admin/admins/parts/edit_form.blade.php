    ...
    <div class="row ">
            ...
            <div class="form-group">
                ...
                <select name="roles[]" multiple class="form-control select2">
                    @foreach (\App\Models\Role::get() as $role)
                        <option
                            @foreach ($admin->roles as $rol)  @if ($rol->id == $role->id) selected
                                @endif @endforeach
        ...
        <div class=" col-md-4 p-2 pt-3">
            <div class="form-group">
                ...
                <input data-validation="required" value="{{ $admin->whats_up_number }}" type="text"
                    class="form-control" id="whats_up_number" name="whats_up_number" placeholder=" ">
            </div>
        </div>
        ...
        <div class=" col-md-4 p-2 pt-3">
            <div class="form-group">
                ...
                <select data-validation="required" name="order_type" class="form-control" id="order_type">
                    <option value="both" {{ $admin->order_type == 'both' ? 'selected' : '' }}>كلاهما (استقدام
                        وايجار)</option>
