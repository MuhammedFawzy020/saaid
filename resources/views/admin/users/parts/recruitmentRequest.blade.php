...
@section('page-title')
    {{ $value === 'serviceMove' ? 'طلب نقل خدمات' : 'طلب استقدام' }}
@endsection
        ...
        @foreach ($cvs as $cv)
            <div class="col-md-4">
                <div class="workerCv card p-2 rounded-3">
                    <a href="{{ route('admins.selectCustomerServiceForCv', [$cv->id, $user->id, $value]) }}" class="d-block">
                        <img src="{{ get_file($cv->cv_file) }}" style=" height: 300px; max-width: 100% ; object-fit: cover"
                            alt="">
                    </a>
                    ...
                    <a href="{{ route('admins.selectCustomerServiceForCv', [$cv->id, $user->id, $value]) }}"
